...
export function isAdjacentTo(a: Maybe<TokenPF2e | TokenDocumentPF2e>, b: Maybe<TokenPF2e | TokenDocumentPF2e>): boolean {
...
export function distanceTo(a: Maybe<TokenPF2e | TokenDocumentPF2e>, b: Maybe<TokenPF2e | TokenDocumentPF2e>): number {
    const tokenA = a instanceof TokenDocument ? (a as TokenDocumentPF2e).object : (a as Maybe<TokenPF2e>);
    const tokenB = b instanceof TokenDocument ? (b as TokenDocumentPF2e).object : (b as Maybe<TokenPF2e>);

    return R.isNonNullish(tokenA) && R.isNonNullish(tokenB) ? tokenA.distanceTo(tokenB) : 0;
}
